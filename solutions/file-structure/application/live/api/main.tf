...
resource "aws_api_gateway_integration" "trump" {
  ...
  type                    = "AWS_PROXY"
  uri                     = data.terraform_remote_state.lambda.outputs.invoke_arn
}
...
resource "aws_lambda_permission" "this" {
  statement_id  = "AllowAPIGatewayInvoke-${data.terraform_remote_state.lambda.outputs.function_name}-${aws_api_gateway_method.trump.resource_id}-${aws_api_gateway_method.trump.http_method}"
